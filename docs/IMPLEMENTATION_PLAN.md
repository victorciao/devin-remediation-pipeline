...
**LANE 1 — CodeQL alerts.** `alert_source = codeql_scan | sarif_file`, default `codeql_scan`: the orchestrator runs a fresh local CodeQL scan of the fork at `base_sha` at the start of every run, and that scan's results are the candidate set the run gates and ranks. `sarif_file` (`fixtures/codeql_alerts.json`) is the SIMULATE
input, so a credential-free offline run stays possible. No source reads the fork's stored analysis. Python only: a path outside `superset/**/*.py` fails `verifiability_exists` with `out_of_scope_frontend`, gating out 3 of the 11 live JS/TS alerts at `a140e74`.
...
`codeql_scan_cache_dir` that is empty or unwritable means scan-every-run. Under `alert_source = codeql_scan` a scan that fails or produces no results is a startup error, never an empty candidate set read as "no debt found". The freshness input to `signal_quality` is the scan timestamp plus the alert's introduction point where
the scan supplies one.
...
**LANE 1 — the alert is gone at head and nothing regressed.** Verification is a local re-scan at the candidate head: `lane1_alert_check = codeql_cli` runs CodeQL over the touched paths, and the alert's `stable_locator` must be absent from it → otherwise `terminal/alert_still_present`. The suite covering `suite_scope` must pass
at head → otherwise `terminal/suite_regressed`. A regression test is required when the alert class admits one; when it does not — `py/overly-large-range` is such a class — `test_nodeid` is null, the row records why, and alert-absence plus suite-green is the whole criterion. Discovery scans at `base_sha` and verification re-scans
at the candidate head; both are scans the orchestrator performs itself, and neither depends on any workflow being enabled on the fork.
...
its Python pack available in the image; Actions enabled with ≥1 completed `pull_request` run; `required_contexts_min` non-empty; issue search reachable when medium-tier candidates are present.
...
4. `lanes/codeql.py`: `alert_source = codeql_scan` as the default — fresh scan at `base_sha`, `base_sha`-keyed cache in `codeql_scan_cache_dir`, startup error on a failed or empty scan, scan-timestamp freshness; `sarif_file` kept as the SIMULATE input, and no stored-analysis source.
...
- `config.py`: `iteration_cap`, `reservation_lease_s`, `DEFAULT_ITERATION_CAP`, `DEFAULT_MAX_SESSIONS` (`max_sessions` keeps its own meaning, §14; `budget_N` default becomes `5`); **keep** `issue_sink`/`IssueSink` and `has_issues`; add `required_contexts_min`, `lane1_alert_check`, `marker_search_enabled`, `codeql_scan_cache_dir`, and the `codeql_scan` member of `alert_source`; drop that key's `api` member and its `GET /code-scanning/alerts` reader.
