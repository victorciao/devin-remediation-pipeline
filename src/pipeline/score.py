"""Pure candidate scoring and observable rubric resolution."""
...
def _int_rows(value: object, label: str) -> dict[str, int]:
    ...
    for key, item in raw_rows.items():
        if not isinstance(item, int) or not 1 <= item <= 5:
            raise ScoreError(f"{label}.{key} must be an integer in 1..5")
        rows[key] = item
    return rows
    ...
    for lane in Lane:
        ...
        for factor_name in (
            ...
            if not isinstance(observable, str) or not observable:
                raise ScoreError(f"{lane.value}.{factor_name}.observable is required")
            if not isinstance(default, int) or not 1 <= default <= 5:
                raise ScoreError(f"{lane.value}.{factor_name}.default must be an integer in 1..5")
...
def _candidate_factor(
    ...
    if row is not None:
        ...
        if value is None:
            raise ScoreError(f"unknown {factor_name} rubric row: {row}")
        return value, row
...
def _lane3_signal_row(candidate: Candidate) -> str | None:
    ...
    try:
        ...
        raise ScoreError("deprecated_in must begin with a major version") from exc
...
def score_candidate(
    ...
    """Score a gate-passed candidate and record each matched rubric row."""
    ...
    risk = _effective_risk(candidate, values["risk"], config)
    raw_score = (
        values["business_impact"]
        * values["verifiability"]
        * values["automatability"]
